...
app.post('/', express.json(), (req, res) => {
    ...
    async function resultq(agent) {
        let cgpa;
        var sql = "SELECT CGPA FROM studentdb WHERE Enrollment_No = " + agent.parameters.number[0];
        if (agent.parameters.number[0] > 92000103000 && agent.parameters.number[0] < 92000103021) {
    ...
    async function fee(agent) {
        var fee_status;
        var resp = '';
        var sql1 = "SELECT Fees FROM studentdb WHERE Enrollment_No = " + agent.parameters.number[0];
        if (agent.parameters.number[0] > 92000103000 && agent.parameters.number[0] < 92000103021) {
    ...
    async function attend(agent) {
        var attendance;
        var resp = '';
        var sql2 = "SELECT Attendance FROM studentdb WHERE Enrollment_No = " + agent.parameters.number[0];
        if (agent.parameters.number[0] > 92000103000 && agent.parameters.number[0] < 92000103021) {
    ...
    async function placement(agent) {
        ...
        var branch;
        var resp = '';
        var sql2 = "SELECT CGPA,Branch FROM studentdb WHERE Enrollment_No = " + agent.parameters.number[0];
        if (agent.parameters.number[0] > 92000103000 && agent.parameters.number[0] < 92000103021) {
            con.query(sql2, function (err, result2) {
                if (err) throw err;
            ...
            const sql3 = "SELECT Companies FROM placement WHERE cgpa > " + cgpa + " AND branch = '" + branch + "'";
            ...
            // wait for the query to finish
            await new Promise(resolve => setTimeout(resolve, 1000));
            resp = "Your eligible companies are " + (companies[0].Companies + ', as your CGPA is ' + cgpa1);
        }
        else {
            resp = "Please enter a valid Enrollment Number!";
        }
...
app.get('/user/:id', (req, res) => {
    ...
    con.connect(function (err) {
        if (err) throw err;
        con.query(`SELECT * FROM studentdb WHERE Class = '${id}'`, function (err, result, fields) {
            if (err) throw err;
            console.log(result);
            res.send(result)
        });
